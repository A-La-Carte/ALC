...
function createCreateVenvTask(name, venvPath, requirementsPath) {
  gulp.task(name, () => {
    if (fs.existsSync(venvPath)) {
      return; // Don't re-do the virtualenv to save time
    }
    const python = commandExists.sync('python3') ? 'python3' : 'python';
    cp.execSync(`${python} -m venv ${venvPath}`);
    const windowsPip = path.resolve(venvPath, 'Scripts', 'pip');
    const linuxPip = path.resolve(venvPath, 'bin', 'pip');

    const pip = fs.existsSync(windowsPip) ? windowsPip : linuxPip;
    cp.execSync(`${pip} install -r ${requirementsPath}`);
  });
}
